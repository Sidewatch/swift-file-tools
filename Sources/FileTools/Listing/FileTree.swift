...
public enum FileTree {
    ...
    public static func ascii(of root: URL, maxDepth: Int = 8, maxEntries: Int = 800) -> String {
        ...
        func walk(_ dir: URL, prefix: String, depth: Int) {
            ...
            let children = DirectoryChildren.sorted(in: dir, skipping: skip)
            for (i, item) in children.enumerated() {
                if count >= maxEntries { lines.append(prefix + Glyph.last + "…"); truncated = true; return }
                let isLast = i == children.count - 1
                let isDir = item.isDirectory
                lines.append(prefix + (isLast ? Glyph.last : Glyph.branch) + item.lastPathComponent + (isDir ? "/" : ""))
                count += 1
                if isDir { walk(item, prefix: prefix + (isLast ? Glyph.blank : Glyph.pipe), depth: depth + 1) }
            }
        }
    ...
    /// The box-drawing pieces of one row.
    private enum Glyph {
        static let branch = "├── ", last = "└── ", pipe = "│   ", blank = "    "
    }
}
